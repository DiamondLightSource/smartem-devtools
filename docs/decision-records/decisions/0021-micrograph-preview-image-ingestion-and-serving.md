...
- `smartem-decisions` #308 is **re-scoped** from "serve `high_res_path`" to this endpoint, and is **gated** behind the ingestion contract and schema change. A new issue tracks the schema migration, contract extension, and consumer persistence (items 1-3); a separate coordination issue tracks the cryoem-services adapter supplying the paths (item 4).
